...
app.get('/auth/google', async (req, res) => {
  ...
  const tokenResponse = await fetch(`https://oauth2.googleapis.com/token`, {
    ...
    body: `code=${code}&client_id=YOUR_GOOGLE_CLIENT_ID&client_secret=YOUR_GOOGLE_CLIENT_SECRET&redirect_uri=http://localhost:3000/auth/google&grant_type=authorization_code`,
  });
...
app.get('/auth/facebook', async (req, res) => {
  const code = req.query.code;
  const tokenResponse = await fetch(`https://graph.facebook.com/v10.0/oauth/access_token?client_id=YOUR_FB_CLIENT_ID&redirect_uri=http://localhost:3000/auth/facebook&client_secret=YOUR_FB_CLIENT_SECRET&code=${code}`);
  const tokenData = await tokenResponse.json();
  res.json(tokenData); // You should handle the token and use it for further user interactions
});
